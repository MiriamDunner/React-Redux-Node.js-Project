...
function AllOrdersPage() {
  ...
  useEffect(() => {
    ...
    if (userTz === 'אנונימי') {
      ...
      setError('אנא התחבר כדי לראות את ההזמנות');
      setLoading(false);
      navigate('/login');
      return;
    }
    ...
    const fetchAllOrders = async () => {
      try {
        console.log('שולף את כל ההזמנות');
        ...
        console.log('כל ההזמנות התקבלו:', data);
        setOrders(data);
        setLoading(false);
      } catch (err) {
  ...
  return (
    <div className="my-orders-container">
      <h2 className="my-orders-title">כל ההזמנות</h2>
      {loading && (
          ...
          {orders.map((order, index) => (
            <li key={order.id} className="order-item" style={{ animationDelay: `${index * 0.1}s` }}>
              <div className="order-details">
                ...
                <p><span className="order-label">תעודת זהות:</span> {order.userId || order.userId}</p>
